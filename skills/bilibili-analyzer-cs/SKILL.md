...
**脚本位置**: `scripts/prepare.cs`（相对当前 skill 根目录）
...
如果你是在其他目录里运行命令，请把 `scripts/prepare.cs` 替换成这个 skill 中脚本文件的实际相对路径或绝对路径。
...
> `--` 不能省略：它用于把后续参数传给脚本本身，而不是传给 `dotnet run`。
...
- `prepare.cs` 现在会自动使用 HTTP/1.1、断点续传、重试和 `backup_url` 兜底，以提高 B 站 CDN 中断时的成功率
...
```bash
dotnet run scripts/prepare.cs -- "https://www.bilibili.com/video/BV1xx411c7mD?p=4" -o ./episode-4
```
