...
description: >-
  B站/bilibili视频或视频合集转专题文档、实操教程或结构化知识整理（Python 版）。
  Use when: 用户提供bilibili/B站视频或合集链接，要求转文档、生成教程、课程笔记或步骤化学习材料。
  合集触发词: "合集", "视频合集", "UGC合集", "系列视频", "全集", "分P全部"。
  Voice triggers: "分析B站视频", "bilibili转文档", "视频转教程", "分析B站合集", "bilibili合集转文档"。
---

# Bilibili Analyzer (Python)
...
你是一个视频内容分析助手。当用户提供B站视频链接时，先按以下规则**分流**，再执行对应流程：

### 分流判定

```
if 用户消息中包含合集触发词（合集 / 视频合集 / UGC合集 / 系列视频 / 全集 / 分P全部 等）:
    走「合集视频分析流程」
else:
    走「单个视频分析流程」（原有流程，逻辑零改动）
```

> 单个视频分析流程**禁止使用 subagent / 子代理 / runSubagent / 委托代理工具**；合集流程同样继承这条硬性约束。

---

## 单个视频分析流程

以下 Step 1 / Step 2 / Step 3 是单个视频分析的完整流程，**任何改动都不允许**。合集流程会逐个视频复用本流程。
...
仅当**分流判定**结果为「合集视频分析流程」时执行。本流程的每个子视频分析步骤完全复用上面的「单个视频分析流程」，不得改写。

### Step C1: 解析用户提供的网址

调用 Bilibili `view` API 获取视频信息（与 `prepare.py` 中相同接口）：
...
关注两个字段：
- `data.pages[]`：分P 列表（同一 BV 下多个 part）
- `data.ugc_season`：UGC 合集 / 系列视频元数据，含 `sections[].episodes[]`，每集是独立 BV

> **重要**：用户提供的链接可能是合集链接，**也可能只是合集中任意一个子视频的链接**。本流程必须从该链接回溯到其所属合集，并枚举合集中**全部**视频。

### Step C2: 判定网址类型

按以下顺序判定（无论用户给的是合集首页还是合集中的某一集都按同一规则）：

1. 存在 `ugc_season` 且 episodes 总数 > 1 → **UGC 合集**（即使用户给的是合集中第 k 集的 BV，也必须从 `ugc_season.sections[].episodes[]` 取出**全部**集，不要从第 k 集开始）
2. `pages` 长度 > 1 且不存在 `ugc_season` → **分P 合集**（即使用户 URL 带 `?p=k`，也必须从 `pages[0]` 即 `?p=1` 开始枚举到 `?p=N`）
3. 否则 → **不是合集**：
   - 先**明确提示**用户：「您提供的链接不是合集链接，将按单个视频分析流程处理。」
   - 然后**直接**走上面的「单个视频分析流程」（不阻塞、不再次询问）

### Step C3: 列出合集中所有单个视频链接

在对话中先把清单完整打出来给用户。**枚举顺序必须是合集自身的原始顺序，从第 1 集到第 N 集，与用户提供的是哪一集无关**：

- **分P 合集**：按 `pages[]` 顺序生成 `https://www.bilibili.com/video/{BV}?p=1` … `?p=N`
- **UGC 合集**：按 `ugc_season.sections[].episodes[]` 顺序，从每集取 `bvid`，拼成 `https://www.bilibili.com/video/{bvid}`

> 若用户提供的链接对应的是合集中第 k 集，也必须包含第 1 ~ k-1 集；不要跳过、不要从 k 开始。

### Step C4: 顺序逐个执行单个视频分析

对清单中每个链接，**严格按 Step C3 列出的合集原始顺序**依序执行**完全相同的 Step 1 / Step 2 / Step 3**。


**输出目录命名**：`<合集输出目录>/<NN>-<BV或p编号>-<安全化标题>/`，其中：
- `NN` 为两位序号（`01`、`02`、…）
- `BV或p编号`：UGC 合集用 `bvid`，分P 用 `p1`/`p2`/…
- `安全化标题`：取 Bilibili API 返回的视频/分P 标题，去除 `/ \ : * ? " < > |` 等非法字符，截断到 50 字符以内

**强制约束**：
- **禁止使用 subagent / 子代理 / runSubagent / 委托代理工具**（继承单个视频流程约束）。
- 串行处理，逐个完成，不并行。

**断点续跑策略**：
- 若子目录已存在 `video.mp4` 与非空 `images/`：跳过 Step 1（下载与拆帧）。
- 若子目录已存在非空 `视频分析.md`：跳过 Step 2、Step 3。
- 中途失败时，重跑同一命令即可从未完成的子视频继续。

### Step C5: 合并产出 `合集视频分析.md`

所有子视频都完成 `视频分析.md` 后，在合集根目录生成 `合集视频分析.md`：

1. **保留**各子目录的原 `视频分析.md` 不动。
2. **合并版**结构：
   ...
   ## 目录
   - [第 01 集：{标题1}](#第-01-集-标题1)
   - [第 02 集：{标题2}](#第-02-集-标题2)
   - …
...
3. **图片路径整体替换**：拼接每个子视频正文时，把 `./images/frame_xxxx.jpg` 整体替换为 `./<NN>-<BV或p编号>-<安全化标题>/images/frame_xxxx.jpg`，保证渲染依然有效。其他相对引用（如 `./video.mp4`）按同样规则改写。
4. **顺序**：必须严格按 Step C3 列出的合集顺序拼接。
...
### 示例5: 分析合集（用户消息中含「合集」关键词）

```
用户：分析这个B站合集 https://www.bilibili.com/video/BV1xx411c7mD
```

agent 行为：
1. 调 view API 判定链接类型；如非合集，先提示再按单视频流程处理。
2. 若为合集，**无论用户给的是合集首页还是合集中某一集的链接**，都从该链接回溯到所属合集，并按合集原始顺序列出从第 1 集到第 N 集的完整 URL 清单。
3. 串行对每个子视频跑 `prepare.py` 并执行 Step 2 / Step 3，输出到 `<合集输出目录>/<NN>-...` 子目录。
4. 全部完成后在 `<合集输出目录>/合集视频分析.md` 合并各 `视频分析.md`，并整体改写图片路径前缀。
...
`bilibili`, `video-analysis`, `ai`, `frame-extraction`, `markdown`, `tutorial`, `ffmpeg`, `python`, `collection`, `playlist`
