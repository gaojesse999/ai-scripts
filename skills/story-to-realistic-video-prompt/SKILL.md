...
输入：一句话点子或一小段剧情。输出：一条可直接投喂视频模型的中文提示词，IMAX真人实拍质感。默认15秒，**2500中文字符左右**（2300-2700区间）。只输出提示词正文，不解释不写大纲。生成后保存为工作区根目录 `场景名-YYYYMMDD-HHMMSS.md`。保存后用本skill目录下的 `scripts/count_chars.py` 验证字数是否达标，不达标则调整后重新保存。
